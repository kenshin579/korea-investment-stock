...
type PriceDetailSnapshot struct {
	...
	Perx   kistypes.Float  `json:"perx"`        // PER
	Pbrx   kistypes.Float  `json:"pbrx"`        // PBR
	...
	TXrat  kistypes.Float  `json:"t_xrat"`      // 원환산당일등락
	PXprc  decimal.Decimal `json:"p_xprc"`      // 원환산전일가격
	PXdif  decimal.Decimal `json:"p_xdif"`      // 원환산전일대비
	PXrat  kistypes.Float  `json:"p_xrat"`      // 원환산전일등락
	TRate  kistypes.Float  `json:"t_rate"`      // 당일환율
	PRate  kistypes.Float  `json:"p_rate"`      // 전일환율
