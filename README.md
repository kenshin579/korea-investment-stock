...
### WebSocket — Phase 11.3 (v1.23.0) — 지수선물옵션 + 상품선물 실시간

지수 4 + 상품 2 = 6 EP. 4 base struct + 2 alias 패턴 (`CommodityFutures*Event = IndexFutures*Event`). 모든 EP 모의 미지원.

| Method | TR_ID | 설명 |
|--------|-------|------|
| `WS.SubscribeIndexFuturesTrade` / `OnIndexFuturesTrade` | H0IFCNT0 | 지수선물 실시간 체결 |
| `WS.SubscribeIndexFuturesAsk` / `OnIndexFuturesAsk` | H0IFASP0 | 지수선물 실시간 호가 (5단계) |
| `WS.SubscribeIndexOptionTrade` / `OnIndexOptionTrade` | H0IOCNT0 | 지수옵션 실시간 체결 (그릭스 + AVRG_VLTL) |
| `WS.SubscribeIndexOptionAsk` / `OnIndexOptionAsk` | H0IOASP0 | 지수옵션 실시간 호가 (5단계) |
| `WS.SubscribeCommodityFuturesTrade` / `OnCommodityFuturesTrade` | H0CFCNT0 | 상품선물 실시간 체결 (지수선물과 alias) |
| `WS.SubscribeCommodityFuturesAsk` / `OnCommodityFuturesAsk` | H0CFASP0 | 상품선물 실시간 호가 (지수선물과 alias) |

### WebSocket — Phase 11.7 (v1.26.0) — 해외선물옵션 실시간

해외선물옵션 실시간 2 EP. 선물/옵션 통합 EP (단일 TR_ID 로 둘 다 수신). 모의 미지원. 그릭스 미포함.

| Method | TR_ID | 설명 |
|--------|-------|------|
| `WS.SubscribeOverseasFuturesTrade` / `OnOverseasFuturesTrade` | HDFFF020 | 해외선물옵션 실시간 체결가 (25 fields) |
| `WS.SubscribeOverseasFuturesAsk` / `OnOverseasFuturesAsk` | HDFFF010 | 해외선물옵션 실시간 호가 (35 fields, BID/ASK 5단계 교차 배열) |
...
### OverseasFutures (해외선물옵션) — Phase 11.5 + 11.6

해외선물 시세 10 EP + 해외옵션 시세 9 EP + 공통 장운영시간 1 = 20 EP. 모든 EP 모의 미지원. base path: `/uapi/overseas-futureoption/v1/`.

#### 해외선물 (Phase 11.5, v1.24.0)

| Go 메서드 | path | TR_ID |
|---|---|---|
| `OverseasFutures.InquirePrice` | `quotations/inquire-price` | HHDFC55010000 |
| `OverseasFutures.StockDetail` | `quotations/stock-detail` | HHDFC55010100 |
| `OverseasFutures.InquireAskingPrice` | `quotations/inquire-asking-price` | HHDFC86000000 |
| `OverseasFutures.SearchContractDetail` | `quotations/search-contract-detail` | HHDFC55200000 |
| `OverseasFutures.InquireTimeFuturechartprice` | `quotations/inquire-time-futurechartprice` | HHDFC55020400 |
| `OverseasFutures.MonthlyCcnl` | `quotations/monthly-ccnl` | HHDFC55020300 |
| `OverseasFutures.DailyCcnl` | `quotations/daily-ccnl` | HHDFC55020100 |
| `OverseasFutures.WeeklyCcnl` | `quotations/weekly-ccnl` | HHDFC55020000 |
| `OverseasFutures.TickCcnl` | `quotations/tick-ccnl` | HHDFC55020200 |
| `OverseasFutures.InvestorUnpdTrend` | `quotations/investor-unpd-trend` | HHDDB95030000 |

#### 해외옵션 + 공통 (Phase 11.6, v1.25.0)

| Go 메서드 | path | TR_ID |
|---|---|---|
| `OverseasFutures.OptPrice` | `quotations/opt-price` | HHDFO55010000 |
| `OverseasFutures.OptDetail` | `quotations/opt-detail` | HHDFO55010100 |
| `OverseasFutures.OptAskingPrice` | `quotations/opt-asking-price` | HHDFO86000000 |
| `OverseasFutures.SearchOptDetail` | `quotations/search-opt-detail` | HHDFO55200000 |
| `OverseasFutures.InquireTimeOptchartprice` | `quotations/inquire-time-optchartprice` | HHDFO55020400 |
| `OverseasFutures.OptMonthlyCcnl` | `quotations/opt-monthly-ccnl` | HHDFO55020300 |
| `OverseasFutures.OptDailyCcnl` | `quotations/opt-daily-ccnl` | HHDFO55020100 |
| `OverseasFutures.OptWeeklyCcnl` | `quotations/opt-weekly-ccnl` | HHDFO55020000 |
